...
var Growl = Class.create({
  ...
  display: function(text){
    this.n++;
    var gid = "growl-alert-"+this.n, opt = Object.extend({image: "asterisk_orange.png",delay: 5, sound: false, autohide:true}, arguments[1] || {});
    ...
    this.keeper.insert({bottom: new Element('div', {'class': "growl-alert",  'id': gid})});
		var me = this;
    $(gid).insert({top: new Element('img',{'class':'growl-icon', 'src': this.mediaFolder+"icons/"+opt.image}), bottom: new Element('p', {'class': 'growl-text'}).update(text)}).setOpacity(0).appear({to: 0.8}).observe('click',function(e) {me.remove(Event.findElement(e,'DIV').id);}).setPngBackground(opt.image,{ backgroundColor: '#000000'});
