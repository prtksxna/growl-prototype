var Growl = Class.create({
  ...
  display: function(text){
    ...
    $(gid).insert({top: new Element('img',{'class':'growl-icon', 'src': this.mediaFolder+"icons/"+opt.image}), bottom: new Element('p', {'class': 'growl-text'}).update(text.capitalize())}).setOpacity(0).appear({to: 0.8}).observe('click',function() {new Effect.DropOut(this.id)}).setPngBackground(opt.image,{ backgroundColor: '#000000'});
